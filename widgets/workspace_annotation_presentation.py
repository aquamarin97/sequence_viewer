...
class WorkspaceAnnotationPresentation:
    def __init__(self, workspace: "SequenceWorkspaceWidget"):
        ...
        self.ann_items: Dict[str, List[AnnotationGraphicsItem]] = {}
    ...
    def rebuild_ann_items(self, layout: RowLayout):
        ...
        for row_index, ann in flat:
            ...
            item = AnnotationGraphicsItem(
                annotation=ann,
                row_index=row_index,
                ann_width=ann.length() * cw,
                ann_height=ann_h,
                on_click=self.workspace._on_ann_item_clicked,
                on_double_click=self.workspace._on_ann_item_double_clicked,
            )
            item.setPos(scene_x, scene_y)
            scene.addItem(item)
            self.ann_items.setdefault(ann.id, []).append(item)

    def update_ann_items_geometry(self, layout: RowLayout):
